...
export default function AdminDashboard() {
  ...
  // URL FISSO PER IL QR CODE
  const targetUrl = "https://comart.vercel.app";
  ...
  // URL API QR CODE PUNTATO AL TUO DOMINIO VERCEL
  ...
  if (loading) return <div className="h-screen bg-slate-50 flex items-center justify-center font-black text-blue-600 uppercase tracking-widest p-10 text-center">Caricamento...</div>;
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
        ...
        <div className="max-w-5xl mx-auto flex flex-col gap-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
               ...
               <button onClick={() => window.print()} className="bg-slate-900 text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg">PDF</button>
            </div>
          </div>
      ...
      <div className="p-4 max-w-5xl mx-auto mt-4">
        ...
        {tab === 'report' && (
          <div className="space-y-8">
            <div className="bg-white p-5 rounded-3xl border shadow-sm print:hidden">
              <label className="text-[10px] font-black text-slate-400 uppercase block mb-2 text-center italic">Seleziona Corso</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-10 border-t">
              {textFeedbacks.map(f => (
                <div key={f.id} className="bg-white p-5 rounded-[2rem] border shadow-sm">
                  ...
                  <p className="text-[11px] text-slate-700 italic font-medium">"{f.commenti}"</p>
                  {f.suggerimenti && <p className="text-[10px] text-slate-400 font-bold border-t pt-2 mt-2">Suggerimento: "{f.suggerimenti}"</p>}
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'accesso' && (
          <div className="flex flex-col items-center justify-center p-8 bg-white rounded-[3rem] border shadow-sm text-center max-w-md mx-auto animate-in zoom-in-95">
            <h2 className="text-2xl font-black mb-2 uppercase italic text-blue-600">DISTRIBUZIONE</h2>
            ...
            <a href={qrImageUrl} target="_blank" rel="noopener noreferrer" className="w-full bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-blue-100 text-center">Download QR Code</a>
            <p className="mt-4 text-[9px] text-slate-300 font-black uppercase italic">{targetUrl}</p>
          </div>
        )}

        {tab === 'domande' && (
          <div className="bg-white rounded-[2.5rem] p-6 border shadow-sm max-w-xl mx-auto">
            <h2 className="text-xl font-black mb-6 uppercase italic text-center text-blue-600">Nuova Domanda</h2>
            <div className="space-y-4 mb-8 bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed">
              <input type="text" placeholder="Scrivi la domanda..." className="w-full p-4 border rounded-2xl text-sm font-bold" value={newQuestion.question_text} onChange={e => setNewQuestion({...newQuestion, question_text: e.target.value})} />
              <div className="flex gap-2">
                <input type="number" className="w-20 p-4 border rounded-2xl text-sm font-bold" value={newQuestion.sort_order} onChange={e => setNewQuestion({...newQuestion, sort_order: parseInt(e.target.value) || 0})} />
                <select className="flex-1 p-4 border rounded-2xl text-sm font-bold bg-white uppercase" value={newQuestion.type} onChange={e => setNewQuestion({...newQuestion, type: e.target.value})}>
                    <option value="valutazione">Voto (1-4)</option>
                    <option value="si_no">Sì / No</option>
                </select>
              </div>
              <button onClick={handleAddQuestion} className="w-full bg-blue-600 text-white p-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg">Salva Domanda</button>
            </div>
            <div className="divide-y px-2">
              {questions.map(q => (
                <div key={q.id} className="flex justify-between items-center py-4 group">
                  ...
                  <button onClick={() => handleDelete('questions', q.id)} className="text-red-400 font-black text-[9px] uppercase opacity-0 group-hover:opacity-100 transition-opacity">Elimina</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'corsi' && (
          <div className="bg-white rounded-[2.5rem] p-6 border shadow-sm max-w-xl mx-auto">
            <h2 className="text-xl font-black mb-6 uppercase italic text-center text-blue-600">Elenco Corsi</h2>
            <div className="space-y-4 mb-8 bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed">
              <input type="text" placeholder="Nome corso..." className="w-full p-4 border rounded-2xl text-sm font-bold" value={newCourse} onChange={e => setNewCourse(e.target.value)} />
              <button onClick={handleAddCourse} className="w-full bg-slate-900 text-white p-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg">Aggiungi Corso</button>
            </div>
            <div className="grid grid-cols-1 gap-2">
              {courses.map(c => (
                <div key={c.id} className="flex justify-between items-center p-4 bg-white border rounded-2xl hover:border-blue-500 transition-all group">
                  ...
                  <button onClick={() => handleDelete('courses', c.id)} className="text-red-300 hover:text-red-600 transition-colors">
