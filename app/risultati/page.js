...
export default function AdminDashboard() {
    ...
    try {
      ...
      const calculatedStats = qData.map(q => {
        ...
        if (q.type === 'valutazione') {
          ['insufficiente', 'sufficiente', 'buono', 'ottimo'].forEach(l => {
  ...
  // Funzione QR Code
  const qrUrl = `https://chart.googleapis.com/chart?cht=qr&chs=300x300&chl=${encodeURIComponent(typeof window !== 'undefined' ? window.location.origin : '')}`;

  if (loading) return <div className="h-screen flex items-center justify-center font-black text-blue-600 bg-slate-50 uppercase p-10 text-center">Caricamento...</div>;
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <div className="p-4 max-w-5xl mx-auto mt-4">
        ...
        {tab === 'report' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="bg-white p-5 rounded-3xl border shadow-sm print:hidden">
              ...
              <select className="w-full p-4 bg-slate-50 border rounded-2xl font-black text-sm" value={filterCourse} onChange={e => setFilterCourse(e.target.value)}>
                    ...
                    ['ottimo', 'buono', 'sufficiente', 'insufficiente'].map(label => (
                      <div key={label} className="space-y-1">
                        ...
                        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div className={`h-full ${label==='ottimo'?'bg-green-500':label==='buono'?'bg-blue-500':label==='sufficiente'?'bg-amber-400':'bg-red-400'}`} style={{ width: `${s.distribution[label]}%` }} />
                        </div>
                      </div>
                    )) : 
                    ['si', 'no'].map(label => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-10">
              {textFeedbacks.map(f => (
        ...
        {/* VIEW: QR CODE */}
        {tab === 'accesso' && (
          <div className="flex flex-col items-center justify-center p-10 bg-white rounded-[3rem] border shadow-sm text-center animate-in zoom-in-95 duration-300">
            <h2 className="text-2xl font-black mb-2 uppercase italic">QR Code Questionario</h2>
            <p className="text-slate-400 text-xs font-bold mb-8 uppercase tracking-widest">Inquadra per compilare il feedback</p>
            <div className="p-6 bg-slate-50 rounded-[2.5rem] border-2 border-dashed mb-8">
              <img src={qrUrl} alt="QR Code Questionario" className="w-64 h-64 shadow-xl rounded-xl" />
            </div>
            <a href={qrUrl} download="qrcode-comart.png" className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-900 transition-all shadow-lg shadow-blue-100">Scarica QR Code</a>
          </div>
        )}
        ...
        {tab === 'domande' && (
          <div className="bg-white rounded-[2.5rem] p-6 border shadow-sm max-w-xl mx-auto">
            <h2 className="text-xl font-black mb-6 uppercase italic text-center text-blue-600">Questionario</h2>
            <div className="space-y-4 mb-8 bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed">
              <input type="text" placeholder="Testo domanda..." className="w-full p-4 border rounded-2xl text-sm font-bold bg-white" value={newQuestion.question_text} onChange={e => setNewQuestion({...newQuestion, question_text: e.target.value})} />
              <div className="flex gap-2">
                <input type="number" className="w-20 p-4 border rounded-2xl text-sm font-bold bg-white" value={newQuestion.sort_order} onChange={e => setNewQuestion({...newQuestion, sort_order: parseInt(e.target.value) || 0})} />
              ...
              <button onClick={handleAddQuestion} className="w-full bg-blue-600 text-white p-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-blue-100">Salva Nuova Domanda</button>
            </div>
            <div className="divide-y px-2">
              {questions.map(q => (
                <div key={q.id} className="flex justify-between items-center py-4 group">
                  ...
                  <button onClick={() => handleDelete('questions', q.id)} className="text-red-400 hover:text-red-600 font-black text-[9px] uppercase transition-colors opacity-0 group-hover:opacity-100">Elimina</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'corsi' && (
          <div className="bg-white rounded-[2.5rem] p-6 border shadow-sm max-w-xl mx-auto">
            ...
            <div className="space-y-4 mb-8 bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed">
              <input type="text" placeholder="Nome corso..." className="w-full p-4 border rounded-2xl text-sm font-bold bg-white" value={newCourse} onChange={e => setNewCourse(e.target.value)} />
              <button onClick={handleAddCourse} className="w-full bg-slate-900 text-white p-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl">Aggiungi Corso all'Elenco</button>
            </div>
            <div className="grid grid-cols-1 gap-2 px-2">
              {courses.map(c => (
                <div key={c.id} className="flex justify-between items-center p-4 bg-white border rounded-2xl hover:border-blue-500 transition-all group">
                  <span className="font-black text-[10px] uppercase text-slate-700">{c.name}</span>
                  <button onClick={() => handleDelete('courses', c.id)} className="text-red-300 hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100">
